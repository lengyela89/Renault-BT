...
static bool sendMessage(void)
{
	...
	while ((periodicRetryCntr < CDCE_MAX_PERIODIC_RETRY__SEND) && (HAL_OK != status))
	{
		periodicRetryCntr++;
		...
		while ((fastRetryCntr < CDCE_MAX_FAST_RETRY__SEND) && (HAL_OK != status))
		{
			...
			status = HAL_UART_Transmit(config.huart, &txMessage[0], messageLength, CDCE_TIMEOUT__SEND);
			if (HAL_OK != status)
			{
				continue;
			}

			status = HAL_UART_Receive(config.huart, rxMessage, 1u, CDCE_TIMEOUT__RECEIVE_ACK);
...
static bool receiveMessage(CDCE_Timeout *timeout)
{
	...
	for (bool runOnce = true; true == runOnce; runOnce = false)
	{
		...
		status = HAL_UART_Receive(config.huart, &rxMessage[1], 2u, CDCE_TIMEOUT__RECEIVE_MESSAGE_FRAGMENT);
		if (HAL_OK != status)
		{
			result = false;
			break;
		...
		status = HAL_UART_Receive(config.huart, &rxMessage[3], rxMessage[CDCEM_IDX__DATA_LEN] + 1u, CDCE_TIMEOUT__RECEIVE_MESSAGE_FRAGMENT);
		...
		status = HAL_UART_Transmit(config.huart, txMessage, 1u, CDCE_TIMEOUT__SEND);
		if (HAL_OK != status)
		{
			result = false;
			break;
...
static bool waitingForMessage(const CDCEM_PayloadType payloadType, CDCE_Timeout *timeout)
{
	if (!receiveMessage(timeout) || (payloadType != rxMessage[CDCEM_IDX__PAYLOAD_TYPE]))
	{
		return false;
	}
...
static void doBooting(void)
{
	...
	/* Send BOOTING */
	txFrameId = 0u;
	cdcemRc = CDCEM_createFrame(txMessage, txFrameId, CDCEM_PAYLOAD_TYPE__BOOTING, 0x60u, 0x06u);
	...
	/* Send BOOT_OK */
	cdcemRc = CDCEM_createFrame(txMessage, ++txFrameId, CDCEM_PAYLOAD_TYPE__BOOT_OK, 0x00, 0x25u);
	if ((CDCEM_RC__OK != cdcemRc) || !sendMessage())
	{
		nextState = CDCE_STATE__ERROR;
	...
	/* Send TRAY_STATUS */
	cdcemRc = CDCEM_createFrame(txMessage, ++txFrameId, CDCEM_PAYLOAD_TYPE__TRAY_STATUS, 0x05u, 0x01u, 0xFCu, 0xFCu);
	if ((CDCEM_RC__OK != cdcemRc) || !sendMessage())
	{
		nextState = CDCE_STATE__ERROR;
	...
	/* Send RANDOM_STATUS */
	cdcemRc = CDCEM_createFrame(txMessage, ++txFrameId, CDCEM_PAYLOAD_TYPE__SET_RANDOM, 0x03u);
	if ((CDCEM_RC__OK != cdcemRc) || !sendMessage())
	{
		nextState = CDCE_STATE__ERROR;
	...
	/* Send STATUS */
	cdcemRc = CDCEM_createFrame(txMessage, ++txFrameId, CDCEM_PAYLOAD_TYPE__STATUS, 0x01u, 0x03u, 0x09u, 0x05u, 0x01u);
	if ((CDCEM_RC__OK != cdcemRc) || !sendMessage())
	{
		nextState = CDCE_STATE__ERROR;
...
static void doStandby(void)
{
	...
	if (receiveMessage(&timeout))
	{
		...
		case CDCEM_PAYLOAD_TYPE__REQ_CD_INFO:
			cdcemRc = CDCEM_createFrame(txMessage, ++txFrameId, CDCEM_PAYLOAD_TYPE__CD_SUMMARY, 0x0Fu, 0x01u, 0x00u, 0x00u, 0x00u, 0x00u);
			if ((CDCEM_RC__OK != cdcemRc) || !sendMessage())
			{
				nextState = CDCE_STATE__ERROR;
			...
			if (receiveMessage(&timeout)
					&& (CDCEM_PAYLOAD_TYPE__START_PLAY == rxMessage[CDCEM_IDX__PAYLOAD_TYPE]))
			{
...
static void doPlaying(void)
{
	...
	cdcemRc = CDCEM_createFrame(txMessage, ++txFrameId, CDCEM_PAYLOAD_TYPE__PLAYING, 0x01u, 0x01u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u);
	if ((CDCEM_RC__OK != cdcemRc) || !sendMessage())
	{
		nextState = CDCE_STATE__ERROR;
...
CDCE_Rc CDCE_configure(CDCE_Config cfg)
{
	...
	{
		return CDCE_RC__CONFIGURING_NOT_ALLOWED;
	}
	...
	if (NULL == cfg.huart)
	{
		...
		return CDCE_RC__INVALID_UART_HANDLE;
	}
